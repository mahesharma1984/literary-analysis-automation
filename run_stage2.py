...
def load_template(template_name):
    """Load template from protocols/templates directory"""
    template_paths = [
    ...
    print(f"⚠️  Warning: Template {template_name} not found, using generated format")
    return None
...
def generate_worksheet(week_package, client):
    ...
    # Load template
    template = load_template("Template_Literary_Analysis_MacroMicro_v2_0.md")
    ...
    # Create prompt
    if template:
        prompt = f"""Fill in this worksheet template with content for Week {week_num}.
...
Follow the template structure exactly. Fill in all sections with appropriate content for high school students.
"""
    else:
        # Fallback to generated format (your current prompt)
